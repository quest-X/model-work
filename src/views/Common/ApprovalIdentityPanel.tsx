...
import {
    APPROVAL_IDENTITY_CHANGED, approvalIdentitySource, clearApprovalIdentity, currentApprovalUser, importApprovalIdentity,
...
export const ApprovalIdentityPanel: React.FC<{zh: boolean}> = ({zh}) => {
    ...
    const [source, setSource] = useState(approvalIdentitySource);
    const [error, setError] = useState('');
    const [busy, setBusy] = useState(false);
    useEffect(() => {
        const update = () => {
            setUser(currentApprovalUser());
            setSource(approvalIdentitySource());
        };
    ...
    return <details hidden={source === 'account'} style={{padding: '8px 12px', gridColumn: '1 / -1'}}>
        <summary>{zh ? '授权身份' : 'Approval identity'}：{user?.user_name || (zh ? '请导入' : 'Import required')}</summary>
        <p>{zh ? '可导入 CLI 生成的临时个人身份文件；私钥仅留在当前页面内存。'
            : 'You may import a temporary CLI-generated identity; its private key stays only in page memory.'}</p>
